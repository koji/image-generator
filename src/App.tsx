...
function App() {
  ...
  // 画像生成の状態を監視
  // App.tsx (修正後のuseEffect)
  useEffect(() => {
    // ★ デバッグログを追加して状態を確認しやすくする
    console.log(`State Check | isGenerating: ${isGenerating}, generatedImage: ${!!generatedImage}, generationError: ${!!generationError}, appState: ${appState}, enhancement: ${enhancement}`);

    if (isGenerating) {
      // ローディング開始時の処理： enhancingPrompt または generatingImage に設定
      const nextState = enhancement ? 'enhancingPrompt' : 'generatingImage';
      // 現在の状態が異なるときだけ更新（不要な再レンダリング防止）
      if (appState !== nextState) {
        console.log(`Setting appState to ${nextState}`);
        setAppState(nextState);
      }
    } else {
      // ローディングが完了した場合 (isGenerating is false)

      if (generatedImage) {
        // ★★★ 修正: 画像生成成功時の遷移条件 ★★★
        // 以前の状態が 'generatingImage' または 'enhancingPrompt' なら 'displayingImage' に遷移
        if (appState === 'generatingImage' || appState === 'enhancingPrompt') {
          console.log(`Image generated, setting appState to displayingImage`);
          setAppState('displayingImage');
        }
        // displayingImage や idle, error からは遷移しない
      } else if (generationError) {
        // ★★★ 修正: エラー発生時の遷移条件 ★★★
        // 以前の状態が 'generatingImage' または 'enhancingPrompt' なら 'error' に遷移
        if (appState === 'generatingImage' || appState === 'enhancingPrompt') {
          console.log(`Generation error detected, setting appState to error`);
          setAppState('error');
        }
        // displayingImage や idle, error からは遷移しない
      } else {
        // ★★★ 修正: 完了したが画像もエラーもない場合のリセット条件 ★★★
        // 以前の状態が 'generatingImage' または 'enhancingPrompt' なら 'idle' に戻す
        // (例: 内部でエラーなく早期リターンした場合など)
        if (appState === 'generatingImage' || appState === 'enhancingPrompt') {
          console.log(`Generation finished without image or error, setting appState to idle`);
          setAppState('idle');
        }
        // displayingImage, error, idle からは遷移しない
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isGenerating, generatedImage, generationError, appState, enhancement]); // enhancement も依存配列に含める
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 font-sans">
      ...
      <div className="flex flex-col items-center mb-4 gap-2">
        <button
          className={`w-full sm:w-auto px-6 py-3 rounded shadow-sm text-white font-medium transition duration-150 ease-in-out ${buttonState.disabled || !browserSupportsSpeechRecognition
